...
#define LASSERT_TYPE(name, args, index, expected_type) \
    LASSERT(args, (args->cell[index]->type == expected_type), \
        "Function '%s' passed incorrect type for argument %d. Got %s, Expected %s.", \
        name, index, ltype_name(args->cell[index]->type), ltype_name(expected_type));

#define LASSERT_ARGC(name, args, expected_argc) \
    LASSERT(args, (args->count == expected_argc), \
        "Function '%s' passed incorrect number of arguments. Got %d, Expected %d.", \
        name, args->count, expected_argc);

#define LASSERT_FULL(name, args, index) LASSERT(args, \
    (args->cell[index]->count != 0), \
    "Function '%s' passed {}.", name);
...
// Function pointer - *lbuiltin takes an lenv* and lval* as inputs and returns an *lval
...
struct lval {
    ...
    /* Function */
    lbuiltin builtin;   // this is a pointer to a function
    lenv* env;
    lval* formals;
    lval* body;
...
lval* lval_err(char* fmt, ...) {
    ...
    /* printf into the error string with a maximum of 511 chatacters */
...
void lval_del(lval* v) {
    ...
    switch (v->type) {
        ...
        case LVAL_SEXPR:
            ...
            /* Also free the memory allocated to containt the pointers */
            free(v->cell);
        break;
        case LVAL_FUN:
            if (!v->builtin) {
...
lval* lval_call(lenv* e, lval* f, lval* a) {
    ...
    /* While argumetns still remain to be processed */
    while (a->count) {
...
lval* lval_eval_sexpr(lenv* e, lval* v) {
    ...
    /* Ensure fist element is a function after evaluation */
    lval* f = lval_pop(v, 0);
    if (f->type != LVAL_FUN) {
